...
| # | Profile | IP 池 | 线程 | 特征 | 适用场景 |
|---|---------|-------|------|------|---------|
| 1 | 优选 /22 | ip_best.txt（11264 IP） | 200 | 历史高分 /22 精选，速度最稳 | 日常优选，稳定性优先 |
| 2 | CF 官方 /22 | ip_expanded_cf22.txt（8192 IP） | 200 | 官方全量 CIDR 展开到 /22 | 官方基础池，覆盖面广 |
| 3 | 历史 /23 | ip_expanded_23.txt（3072 IP） | 160 | 历史 /24 向上聚合一级 | 在精度和覆盖之间折中 |
| 4 | 历史 /24 | ip_history_expanded.txt（2560 IP） | 150 | 历史高分 IP 所在 /24 全展开 | 细粒度精准优选 |
| 5 | 双倍合并池 | ip_double.txt | 1000 | 多源 IP 合并扩充，大并发 | 计划任务默认池，大批量扫 |
| 6 | 全量原始池 | ip_full.txt | 300 | 原始基线池，sl=0.5 | 兜底场景，放宽速度门槛 |
